...
import refundsListApi, { RefundsListProps } from '@api/refund/list';
import refundsDeleteApi, { RefundsDeleteProps } from '@api/refund/delete';
...
import { header, columns } from './refunds-columns';
...
const Refunds = () => {
  ...
  const {
    ...
    getCallbackData: (props: RefundsListProps) => refundsListApi({ ...props }),
  });
  const {
    setData,
    loading: deleteLoading,
  } = useUpdate({
    getCallbackData: (props: RefundsDeleteProps) => refundsDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Refund has been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Refunds</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.catalog.refunds.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
      ...
      <DataTable
        ...
        total={total || 50}
        mobileColumns={['name', 'slug', 'operations']}
      />
      <Filter
        isShow={isShow}
