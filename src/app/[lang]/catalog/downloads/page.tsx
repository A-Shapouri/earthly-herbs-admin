    ...
    () => [
      ...
      },
      {
        ...
        cell: (info) => {
          return (
            <Div className={'gap-2'}>
              <Button href={getParseRoute({ pathname: routes['route.catalog.downloads.update'], query: { id: info.row.original.name }, locale: 'en' })} size={'small'} variant={'filled'} className={'text-nowrap !bg-blue-500 !text-white hover:!bg-blue-700 active:!bg-blue-300'} rounded='small' shape='square' startAdornment={<EditIcon className={'h-4 w-4'} />} color={'frost'} />
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Downloads</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-between w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.catalog.downloads.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
