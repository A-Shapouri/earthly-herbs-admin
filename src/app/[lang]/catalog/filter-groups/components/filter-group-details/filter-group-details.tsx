...
import filterGroupsStoreApi, { FilterGroupsStoreProps } from '@api/filter-groups/store';
import { useModuleForm } from '@modules/catalog-form/catalog-store';
import useFetch from '@hooks/use-fetch';
import filterGroupsShowApi from '@api/filter-groups/show';
import filterGroupsUpdateApi, { FilterGroupsUpdateProps } from '@api/filter-groups/update';
...
const FilterGroupDetails = () => {
  ...
  const {
    ...
    getCallbackData: (props: FilterGroupsStoreProps) => filterGroupsStoreApi({ ...props }),
    apiMessageText: 'Store Succeeded',
    apiMessageDescription: 'A new Filter Group has been added successfully.',
  });
  ...
  const {
    ...
    getCallbackData: (props: FilterGroupsUpdateProps) => filterGroupsUpdateApi({ ...props }),
    apiMessageText: 'Update Succeeded',
    apiMessageDescription: 'The Filter Group has been updated successfully.',
  });
  ...
  const {
    ...
    getCallbackData: () => filterGroupsShowApi({ id: id }),
  });
      ...
      callback: (state) => {
        if (!state.error.errorFlag) {
          storeData({
            ...
            router.push(getParseRoute({ pathname: routes['route.catalog.filter-groups.update'], query: { id: response.id } }));
          });
        }
      },
    });
  ...
  return (
    <Div className='flex-col justify-center w-full gap-4 md:gap-8'>
      <Div className={'w-full gap-2 md:gap-4 md:justify-end justify-between'}>
        <Button href={getParseRoute({ pathname: routes['route.catalog.filter-groups.index'], locale: lang })} rounded={'small'} size={'small'} color={'slate'} startAdornment={<RedoIcon />}>Return to List</Button>
        {id ? (
