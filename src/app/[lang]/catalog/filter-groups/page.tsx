...
import filterGroupsDeleteApi, { FilterGroupsDeleteProps } from '@api/filter-groups/delete';
...
import { header, columns } from './filter-groups-columns';
...
const FilterGroups = () => {
  ...
  const {
    ...
    getCallbackData: (props: FilterGroupsListProps) => filterGroupsListApi({ ...props }),
  });
  const {
    setData,
    loading: deleteLoading,
  } = useUpdate({
    getCallbackData: (props: FilterGroupsDeleteProps) => filterGroupsDeleteApi({ ...props }),
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Filter Groups</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.catalog.filter-groups.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
