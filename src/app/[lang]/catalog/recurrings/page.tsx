...
import recurringsListApi, { RecurringsListProps } from '@api/recurrings/list';
import recurringsDeleteApi, { RecurringsDeleteProps } from '@api/recurrings/delete';
...
import { header, columns } from './recurrings-columns';
...
const Recurrings = () => {
  ...
  const {
    ...
    getCallbackData: (props: RecurringsListProps) => recurringsListApi({ ...props }),
  });
  const {
    setData,
    loading: deleteLoading,
  } = useUpdate({
    getCallbackData: (props: RecurringsDeleteProps) => recurringsDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Recurring has been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Recurring</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.catalog.recurrings.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
      ...
      <DataTable
        ...
        total={total || 50}
        mobileColumns={['name', 'cycle', 'price']}
      />
      <Filter
        isShow={isShow}
