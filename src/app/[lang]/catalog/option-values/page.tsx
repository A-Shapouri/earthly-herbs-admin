...
import optionValuesListApi, { OptionValuesListProps } from '@api/option-values/list';
import optionValuesDeleteApi, { OptionsDeleteProps } from '@api/option-values/delete';
...
import { header, columns } from './option-values-columns';
...
const OptionValues = () => {
  ...
  const {
    ...
    getCallbackData: (props: OptionValuesListProps) => optionValuesListApi({ ...props }),
  });
  const {
    setData,
    loading: deleteLoading,
  } = useUpdate({
    getCallbackData: (props: OptionsDeleteProps) => optionValuesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Option Value has been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Option Values</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.catalog.option-values.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
