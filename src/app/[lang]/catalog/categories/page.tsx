...
import categoriesListApi, { CategoriesListProps } from '@api/categories/list';
import categoriesDeleteApi, { CategoriesDeleteProps } from '@api/categories/delete';
...
import { header, columns } from './categories-columns';
...
const Categories = () => {
  ...
  const {
    ...
    getCallbackData: (props: CategoriesListProps) => categoriesListApi({ ...props }),
  });
  const {
    setData,
    loading: deleteLoading,
  } = useUpdate({
    getCallbackData: (props: CategoriesDeleteProps) => categoriesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Category has been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      ...
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.catalog.categories.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
