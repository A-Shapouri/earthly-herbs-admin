...
import filtersDeleteApi, { FiltersDeleteProps } from '@api/filters/delete';
...
import { header, columns } from './filters-columns';
...
const Filters = () => {
  ...
  const {
    ...
    getCallbackData: (props: FiltersListProps) => filtersListApi({ ...props }),
  });
  const {
    setData,
    loading: deleteLoading,
  } = useUpdate({
    getCallbackData: (props: FiltersDeleteProps) => filtersDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Filter has been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Filter</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.catalog.filters.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
      ...
      <DataTable
        ...
        total={total || 50}
        mobileColumns={['name', 'filterGroupId', 'operations']}
      />
      <Filter
        isShow={isShow}
