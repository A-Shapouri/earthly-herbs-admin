...
import attributesDeleteApi, { AttributesDeleteProps } from '@api/attributes/delete';
...
import { header, columns } from './attributes-columns';
...
const Attributes = () => {
  ...
  const {
    ...
    getCallbackData: (props: AttributesListProps) => attributesListApi({ ...props }),
  });
  const {
    setData,
    loading: deleteLoading,
  } = useUpdate({
    getCallbackData: (props: AttributesDeleteProps) => attributesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Attribute has been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Attribute</Text>
      </Div>
