...
import productsListApi, { ProductsListProps } from '@api/products/list';
import productsDeleteApi, { ProductsDeleteProps } from '@api/products/delete';
...
import { header, columns } from './products-columns';
...
const Products = () => {
  ...
  const {
    ...
    getCallbackData: (props: ProductsListProps) => productsListApi({ ...props }),
  });
  const {
    setData,
    loading: deleteLoading,
  } = useUpdate({
    getCallbackData: (props: ProductsDeleteProps) => productsDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Product has been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Product</Text>
      </Div>
