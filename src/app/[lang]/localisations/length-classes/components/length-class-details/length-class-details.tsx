...
import lengthClassesShowApi from '@api/length-classes/show';
import lengthClassesStoreApi, { LengthClassesStoreProps } from '@api/length-classes/store';
import lengthClassesUpdateApi, { LengthClassesUpdateProps } from '@api/length-classes/update';
...
const LengthClassDetails = () => {
  ...
  const {
    ...
    getCallbackData: () => lengthClassesShowApi({ id: id }),
  });
  ...
  const {
    ...
    getCallbackData: (props: LengthClassesStoreProps) => lengthClassesStoreApi({ ...props }),
    apiMessageText: 'Store Succeeded',
    apiMessageDescription: 'A new Length Class has been added successfully.',
  });
  ...
  const {
    ...
    getCallbackData: (props: LengthClassesUpdateProps) => lengthClassesUpdateApi({ ...props }),
    apiMessageText: 'Update Succeeded',
    apiMessageDescription: 'The Length Class has been updated successfully.',
  });
    ...
    if (!error) {
      storeData({
        ...
        router.push(getParseRoute({
          pathname: routes['route.localisations.lengthClasses.update'],
          query: {
            id: response.id,
          },
          locale: lang,
  ...
  return (
    <Div className='flex-col justify-center w-full gap-4 md:gap-8'>
      <Div className={'w-full gap-2 md:gap-4 md:justify-end justify-between'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.lengthClasses.index'], locale: lang })}
          rounded={'small'}
          size={'small'}
          color={'slate'}
            ...
            <Div className={'w-full gap-6 grid md:grid-cols-3 md:grid-rows-3 grid-cols-1 mt-4 grid-flow-dense'}>
              <MainSection className='md:row-span-1 md:col-span-2' title='Primary Details'>
                <TextField
                  ...
                  label={'Length Class Value'}
                  helperText={state.error.valueError ? 'Length Class Value is required!' : undefined}
                />
              </MainSection>
