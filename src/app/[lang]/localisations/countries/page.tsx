...
import countriesListApi, { CountriesListProps } from '@api/countries/list';
import countriesDeleteApi, { CountriesDeleteProps } from '@api/countries/delete';
...
import { header, columns } from './countries-columns';
...
const Countries = () => {
  ...
  const {
    ...
    getCallbackData: (props: CountriesListProps) => countriesListApi({ ...props }),
  });
  const {
    setData,
    loading: deleteLoading,
  } = useUpdate({
    getCallbackData: (props: CountriesDeleteProps) => countriesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The country has been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Countries</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.countries.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
      ...
      <DataTable
        ...
        previousPage={() => {}}
        total={total || 50}
        mobileColumns={['name', 'addressFormat', 'operations']}
      />
      <Filter
        isShow={isShow}
