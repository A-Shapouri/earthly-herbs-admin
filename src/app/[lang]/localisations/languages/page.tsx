...
import languagesDeleteApi, { LanguagesDeleteProps } from '@api/languages/delete';
...
import { header, columns } from './language-columns';
...
const Languages = () => {
  ...
  const {
    ...
    getCallbackData: (props: LanguagesListProps) => languagesListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: LanguagesDeleteProps) => languagesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The language as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Languages</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.languages.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
