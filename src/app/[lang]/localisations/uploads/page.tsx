...
import uploadsListApi, { UploadsListProps } from '@api/uploads/list';
import uploadsDeleteApi, { UploadsDeleteProps } from '@api/uploads/delete';
...
import { header, columns } from './upload-columns';
...
const Uploads = () => {
  ...
  const {
    ...
    getCallbackData: (props: UploadsListProps) => uploadsListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: UploadsDeleteProps) => uploadsDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The upload as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Uploads</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.uploads.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
      ...
      <DataTable
        ...
        previousPage={() => {}}
        total={total || 50}
        mobileColumns={['name', 'code', 'operations']}
      />
      <Filter
        isShow={isShow}
