...
import zonesListApi, { ZonesListProps } from '@api/zones/list';
import zonesDeleteApi, { ZonesDeleteProps } from '@api/zones/delete';
...
import { header, columns } from './zone-columns';
...
const Zones = () => {
  ...
  const {
    ...
    getCallbackData: (props: ZonesListProps) => zonesListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: ZonesDeleteProps) => zonesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The zone as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Zones</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.zones.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
