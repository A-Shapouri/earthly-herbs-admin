...
import taxRatesListApi, { TaxRatesListProps } from '@api/tax-rates/list';
import taxRatesDeleteApi, { TaxRatesDeleteProps } from '@api/tax-rates/delete';
...
import { header, columns } from './tax-rate-columns';
...
const TaxRates = () => {
  ...
  const {
    ...
    getCallbackData: (props: TaxRatesListProps) => taxRatesListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: TaxRatesDeleteProps) => taxRatesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Tax Rate as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Tax Rates</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.taxRates.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
      ...
      <DataTable
        ...
        previousPage={() => {}}
        total={total || 50}
        mobileColumns={['name', 'rate', 'operations']}
      />
      <Filter
        isShow={isShow}
