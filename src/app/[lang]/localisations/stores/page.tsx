...
import storesDeleteApi, { StoresDeleteProps } from '@api/stores/delete';
...
import { header, columns } from './store-columns';
...
const Stores = () => {
  ...
  const {
    ...
    getCallbackData: (props: StoresListProps) => storesListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: StoresDeleteProps) => storesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The store as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Stores</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.stores.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
