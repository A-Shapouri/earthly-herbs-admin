...
export const columns: ColumnDef<Person>[] = [
  {
    ...
    header: header.code,
    id: 'code',
    cell: info => <Text align='center' typography={['xs', 'xs']}>{info.row.original.code}</Text>,
  },
  {
    accessorFn: row => row.key,
    header: header.key,
    id: 'key',
    cell: info => <Text align='center' typography={['xs', 'xs']}>{info.row.original.key}</Text>,
  },
  {
    accessorFn: row => row.serialized,
    header: header.serialized,
    id: 'serialized',
    cell: info => {
      if (info.row.original.serialized) {
        return (
