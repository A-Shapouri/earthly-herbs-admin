...
import shippingCouriersListApi, { ShippingCouriersListProps } from '@api/shipping-couriers/list';
import shippingCouriersDeleteApi, { ShippingCouriersDeleteProps } from '@api/shipping-couriers/delete';
...
import { header, columns } from './shipping-courier-columns';
...
const ShippingCouriers = () => {
  ...
  const {
    ...
    getCallbackData: (props: ShippingCouriersListProps) => shippingCouriersListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: ShippingCouriersDeleteProps) => shippingCouriersDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Shipping Courier as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Shipping Couriers</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.shippingCouriers.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
