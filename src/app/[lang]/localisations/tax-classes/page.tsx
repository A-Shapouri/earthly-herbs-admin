...
import taxClassesDeleteApi, { TaxClassesDeleteProps } from '@api/tax-classes/delete';
...
import { header, columns } from './tax-class-columns';
...
const TaxClasses = () => {
  ...
  const {
    ...
    getCallbackData: (props: TaxClassesListProps) => taxClassesListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: TaxClassesDeleteProps) => taxClassesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Tax Class as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Tax Classes</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.taxClasses.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
