...
import geoZonesShowApi from '@api/geo-zones/show';
import geoZonesStoreApi, { GeoZonesStoreProps } from '@api/geo-zones/store';
import geoZonesUpdateApi, { GeoZonesUpdateProps } from '@api/geo-zones/update';
...
const GeoZoneDetails = () => {
  ...
  const {
    ...
    getCallbackData: () => geoZonesShowApi({ id: id }),
  });
  ...
  const {
    ...
    getCallbackData: (props: GeoZonesStoreProps) => geoZonesStoreApi({ ...props }),
    apiMessageText: 'Store Succeeded',
    apiMessageDescription: 'A new Geo Zone has been added successfully.',
  });
  ...
  const {
    ...
    getCallbackData: (props: GeoZonesUpdateProps) => geoZonesUpdateApi({ ...props }),
    apiMessageText: 'Update Succeeded',
    apiMessageDescription: 'The Geo Zone has been updated successfully.',
  });
    ...
    if (!error) {
      geoZoneData({
        ...
        router.push(getParseRoute({
          pathname: routes['route.localisations.geoZones.update'],
          query: {
            id: response.id,
          },
          locale: lang,
  ...
  return (
    <Div className='flex-col justify-center w-full gap-4 md:gap-8'>
      <Div className={'w-full gap-2 md:gap-4 md:justify-end justify-between'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.geoZones.index'], locale: lang })}
          rounded={'small'}
          size={'small'}
          color={'slate'}
          loading={loading || geoZoneLoading || updateLoading}
          disabled={loading || geoZoneLoading || updateLoading}
            ...
            <Div className={'w-full gap-6 grid md:grid-cols-3 md:grid-rows-3 grid-cols-1 mt-4 grid-flow-dense'}>
              <MainSection className='md:row-span-1 md:col-span-2' title='Primary Details'>
                <TextField
                  ...
                  label={'Geo Zone Name'}
                  helperText={state.error.nameError ? 'Geo Zone Name is required!' : undefined}
                />
                <TextField
                  disabled={loading || geoZoneLoading || updateLoading}
                  size='small'
                  ...
                  label={'Geo Zone Description'}
                  helperText={state.error.descriptionError ? 'Geo Zone Description is required!' : undefined}
                />
              </MainSection>
