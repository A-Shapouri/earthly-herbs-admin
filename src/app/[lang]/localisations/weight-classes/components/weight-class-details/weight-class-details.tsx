...
import weightClassesShowApi from '@api/weight-classes/show';
import weightClassesStoreApi, { WeightClassesStoreProps } from '@api/weight-classes/store';
import weightClassesUpdateApi, { WeightClassesUpdateProps } from '@api/weight-classes/update';
...
const WeightClassDetails = () => {
  ...
  const {
    ...
    getCallbackData: () => weightClassesShowApi({ id: id }),
  });
  ...
  const {
    ...
    getCallbackData: (props: WeightClassesStoreProps) => weightClassesStoreApi({ ...props }),
    apiMessageText: 'Store Succeeded',
    apiMessageDescription: 'A new Weight Class has been added successfully.',
  });
  ...
  const {
    ...
    getCallbackData: (props: WeightClassesUpdateProps) => weightClassesUpdateApi({ ...props }),
    apiMessageText: 'Update Succeeded',
    apiMessageDescription: 'The Weight Class has been updated successfully.',
  });
    ...
    if (!error) {
      storeData({
        ...
        router.push(getParseRoute({
          pathname: routes['route.localisations.weightClasses.update'],
          query: {
            id: response.id,
          },
          locale: lang,
  ...
  return (
    <Div className='flex-col justify-center w-full gap-4 md:gap-8'>
      <Div className={'w-full gap-2 md:gap-4 md:justify-end justify-between'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.weightClasses.index'], locale: lang })}
          rounded={'small'}
          size={'small'}
          color={'slate'}
            ...
            <Div className={'w-full gap-6 grid md:grid-cols-3 md:grid-rows-3 grid-cols-1 mt-4 grid-flow-dense'}>
              <MainSection className='md:row-span-1 md:col-span-2' title='Primary Details'>
                <TextField
                  ...
                  helperText={state.error.nameError ? 'Weight Class Name is required!' : undefined}
                />
              </MainSection>
