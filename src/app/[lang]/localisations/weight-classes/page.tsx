...
import weightClassesDeleteApi, { WeightClassesDeleteProps } from '@api/weight-classes/delete';
...
import { header, columns } from './weight-class-columns';
...
const WeightClasses = () => {
  ...
  const {
    ...
    getCallbackData: (props: WeightClassesListProps) => weightClassesListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: WeightClassesDeleteProps) => weightClassesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Weight Class as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Weight Classes</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.weightClasses.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
