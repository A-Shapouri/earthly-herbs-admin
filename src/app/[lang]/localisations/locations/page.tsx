...
import locationsDeleteApi, { LocationsDeleteProps } from '@api/locations/delete';
...
import { header, columns } from './location-columns';
...
const Locations = () => {
  ...
  const {
    ...
    getCallbackData: (props: LocationsListProps) => locationsListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: LocationsDeleteProps) => locationsDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The location as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Locations</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.locations.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
      ...
      <DataTable
        ...
        total={total || 50}
        mobileColumns={['name', 'telephone', 'operations']}
      />
      <Filter
        isShow={isShow}
