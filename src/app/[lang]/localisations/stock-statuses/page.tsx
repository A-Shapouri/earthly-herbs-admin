...
import stockStatusesDeleteApi, { StockStatusesDeleteProps } from '@api/stock-statuses/delete';
...
import { header, columns } from './stock-status-columns';
...
const StockStatuses = () => {
  ...
  const {
    ...
    getCallbackData: (props: StockStatusesListProps) => stockStatusesListApi({ ...props }),
  });
  ...
  const {
    ...
    getCallbackData: (props: StockStatusesDeleteProps) => stockStatusesDeleteApi({ ...props }),
    apiMessageText: 'Delete Succeeded',
    apiMessageDescription: 'The Stock Status as been Deleted Successfully.',
  });
  ...
  return (
    <Div className={'flex-col justify-center w-full gap-4 md:gap-8'}>
      <Div className={'flex-col sm:flex-row w-full justify-between gap-4'}>
        ...
        />
        <Text type={'bold'} typography={['xl', 'xl']} align={'start'}>Stock Statuses</Text>
      </Div>
      <Div className={'self-end gap-x-4 justify-end w-full md:w-auto'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.stockStatuses.create'], locale: 'en' })}
          className={'w-32'}
          rounded={'small'}
          size={'small'}
          color={'slate'}
