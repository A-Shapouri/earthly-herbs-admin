...
import stockStatusesShowApi from '@api/stock-statuses/show';
import stockStatusesStoreApi, { StockStatusesStoreProps } from '@api/stock-statuses/store';
import stockStatusesUpdateApi, { StockStatusesUpdateProps } from '@api/stock-statuses/update';
...
const StockStatusDetails = () => {
  ...
  const checkValidation = () => {
    return (!state.data.name && !state.data.languageId);
  };
  ...
  const {
    ...
    getCallbackData: () => stockStatusesShowApi({ id: id }),
  });
  ...
  const {
    ...
    getCallbackData: (props: StockStatusesStoreProps) => stockStatusesStoreApi({ ...props }),
    apiMessageText: 'Store Succeeded',
    apiMessageDescription: 'A new StockStatus has been added successfully.',
  });
  ...
  const {
    ...
    getCallbackData: (props: StockStatusesUpdateProps) => stockStatusesUpdateApi({ ...props }),
    apiMessageText: 'Update Succeeded',
    apiMessageDescription: 'The StockStatus has been updated successfully.',
  });
    ...
    if (!error) {
      storeData({
        ...
        router.push(getParseRoute({
          pathname: routes['route.localisations.stockStatuses.update'],
          query: {
            id: response.id,
          },
          locale: lang,
  ...
  const handleGetLanguages = (value: string) => {
    handleChangeValue({
      key: 'languageId',
      value: value,
    });
  ...
  return (
    <Div className='flex-col justify-center w-full gap-4 md:gap-8'>
      <Div className={'w-full gap-2 md:gap-4 md:justify-end justify-between'}>
        <Button
          href={getParseRoute({ pathname: routes['route.localisations.stockStatuses.index'], locale: lang })}
          rounded={'small'}
          size={'small'}
          color={'slate'}
            ...
            <Div className={'w-full gap-6 grid md:grid-cols-3 md:grid-rows-3 grid-cols-1 mt-4 grid-flow-dense'}>
              <MainSection className='md:row-span-1 md:col-span-2' title='Primary Details'>
                <TextField
                  ...
                  label={'StockStatus Name'}
                  helperText={state.error.nameError ? 'StockStatus Name is required!' : undefined}
                />
                <Select
                  rounded='small'
                  value={state?.data?.languageId?.toString()}
                  size='small'
                  className='w-full md:col-span-3'
                  disabled={loading || storeLoading || updateLoading || languagesLoading}
                  label={'Country'}
                  optionsList={languagesData}
                  onChange={(newValue) => handleGetLanguages(newValue)}
                  id={'id'}
                  text={'name'}/>
              </MainSection>
